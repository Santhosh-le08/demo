document.addEventListener("DOMContentLoaded", function () {
    ...
    setInterval(() => {
        ...
        if (lowStockItems.length > 0 || expiringItems.length > 0) {
            statusMessage = `<div class="alert-item warning">
                <i class="fas fa-exclamation-triangle"></i>
                <p>⚠️ Inventory Alert: ${lowStockItems.length} items low in stock, ${expiringItems.length} items expiring soon</p>
            </div>`;
        } else {
            statusMessage = `<div class="alert-item success">
                <i class="fas fa-check-circle"></i>
                <p>✅ All Clear: Your inventory is well-stocked and up to date!</p>
            </div>`;
        }

        // Update the alerts container
        alertsContainer.innerHTML = statusMessage;
...
function startScanning() {
    showAlert('Barcode scanning functionality coming soon!', 'info');
}
...
function showSmartSuggestions() {
    showAlert('AI insights functionality coming soon!', 'info');
}

// Show Shopping List
function showShoppingList() {
    showAlert('Shopping list functionality coming soon!', 'info');
}
...
async function handleAddItem(e) {
    ...
    const newItem = {
        ...
        status: getItemStatus(formData.get('expiryDate'))
    };
    
    inventory[newItem.category].push(newItem);
    saveInventory();
    updateInventoryDisplay();
    closeAddItemModal();
}
...
function editItem(id) {
    ...
    // Remove old item
    inventory[item.category] = inventory[item.category].filter(item => item.id !== id);
    saveInventory();
    updateInventoryDisplay();
}
...
function deleteItem(id) {
    if (confirm('Are you sure you want to delete this item?')) {
        inventory.filter(item => item.id === id)[0].status = 'expired';
        saveInventory();
        updateInventoryDisplay();
    }
}
